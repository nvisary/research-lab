...
def _signals_for_symbol(df: pd.DataFrame, params: dict, symbol: str) -> pd.Series:
    ...
    for i in range(len(df)):
        if curr_pos == 0:
            ...
            # Exit if price hits P OR CCI crosses 0 upwards OR new Short signal
            if close.iloc[i] >= p.iloc[i] or cci.iloc[i] >= 0 or short_cond.iloc[i]:
                curr_pos = 0.0
                if not long_only and short_cond.iloc[i]:
                    curr_pos = -1.0
        elif curr_pos == -1.0:
            # Exit if price hits P OR CCI crosses 0 downwards OR new Long signal
            if close.iloc[i] <= p.iloc[i] or cci.iloc[i] <= 0 or long_cond.iloc[i]:
                curr_pos = 0.0
                if long_cond.iloc[i]:
                    curr_pos = 1.0
