"""1m pump/dump shock fade baseline.

Detect abnormal one-minute return plus abnormal volume, then fade the shock
after a configurable delay. This is intentionally simple: fixed hold, small raw
position sizing, and no exchange-specific execution logic inside the strategy.
The research-lab harness supplies costs, funding adjustment, walk-forward
splits, audit, trade ledger, and diagnostics.
"""
...
def _positions_for_symbol(symbol: str, df: pd.DataFrame, params: dict) -> pd.Series:
    ...
    pump = (ret_z >= return_z) & (vol_z >= volume_z) & shock_big_enough & liquid_enough & flat_high_vol
    ...
    allowed_reclaim_symbol = symbol in {"ETHUSDT", "DOGEUSDT", "ARBUSDT", "WLDUSDT", "1000PEPEUSDT"}
